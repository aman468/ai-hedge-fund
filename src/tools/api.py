...
# ── Routing ───────────────────────────────────────────────────────────────────
# Automatically use yfinance for Indian market tickers (NSE/BSE) or when the
# DATA_PROVIDER env var is set to "yfinance".

def _use_yfinance(ticker: str) -> bool:
    t = ticker.upper()
    return (
        t.endswith(".NS")  # NSE — e.g. RELIANCE.NS
        or t.endswith(".BO")  # BSE — e.g. 500325.BO
        or os.environ.get("DATA_PROVIDER", "").lower() == "yfinance"
    )
...
def get_insider_trades(
    ...
    if _use_yfinance(ticker):
        return _yf().get_insider_trades(ticker, end_date, start_date, limit)
    # Create a cache key that includes all parameters to ensure exact matches
...
def get_company_news(
    ...
    if _use_yfinance(ticker):
        return _yf().get_company_news(ticker, end_date, start_date, limit)
    # Create a cache key that includes all parameters to ensure exact matches
    cache_key = f"{ticker}_{start_date or 'none'}_{end_date}_{limit}"
